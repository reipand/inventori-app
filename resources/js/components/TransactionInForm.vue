<template>
  <div class="max-w-lg space-y-5">
    <div>
      <h2 class="text-lg font-semibold">Transaksi Masuk</h2>
      <p class="text-sm text-muted-foreground">Catat penerimaan barang dari supplier</p>
    </div>

    <!-- Low stock warning -->
    <Transition name="fade">
      <div v-if="lowStockWarning" class="flex items-start gap-3 rounded-xl bg-yellow-50 border border-yellow-200 px-4 py-3 text-sm text-yellow-800">
        <span class="shrink-0 mt-0.5">⚠️</span>
        <div class="flex-1">
          <p class="font-medium">Peringatan Stok Rendah</p>
          <p>Stok <strong>{{ lastProductName }}</strong> berada di bawah atau sama dengan stok minimum.</p>
        </div>
        <button @click="lowStockWarning = false" class="shrink-0 opacity-60 hover:opacity-100">✕</button>
      </div>
    </Transition>
    ...
    <div class="rounded-xl border bg-card p-6 space-y-5">
      ...
      <div class="field">
        ...
        <input v-model="form.supplier_name" type="text" class="input-base" placeholder="PT Supplier" :disabled="submitting" />
      </div>

      <div class="field">
        <label class="field-label">Harga Beli per Unit (Rp) <span class="req">*</span></label>
      ...
      <div v-if="currentStock !== null" class="rounded-lg bg-primary/5 border border-primary/20 px-4 py-3 text-sm">
        Stok terbaru: <strong class="text-primary">{{ currentStock }}</strong> {{ selectedUnit }}
      </div>
...
import { createTransactionIn } from '@/services/transactionService';
import { useToast } from '@/composables/useToast';
import { useNotificationStore } from '@/stores/notification';

const emit = defineEmits<{
  success: [payload: { productName: string; quantity: number; currentStock: number }];
}>();
...
const form = ref({ product_id: '', quantity: '', transaction_date: today(), supplier_name: '', price_per_unit: '' });

const selectedProduct = computed(() => products.value.find((p) => p.id === form.value.product_id) ?? null);
const selectedUnit = computed(() => selectedProduct.value?.unit ?? '');
...
function validate() {
  ...
  if (!form.value.supplier_name.trim()) return 'Nama supplier wajib diisi.';
  const price = Number(form.value.price_per_unit);
  if (form.value.price_per_unit === '' || isNaN(price) || price < 0) return 'Harga beli tidak valid.';
  return null;
}

async function submitForm() {
  formError.value = ''; successMsg.value = ''; lowStockWarning.value = false; currentStock.value = null;
  const err = validate();
  ...
  try {
    ...
    const qty = Number(form.value.quantity);
    const result = await createTransactionIn({
    ...
    if (result.low_stock_warning) {
      lastProductName.value = product?.name ?? '';
      lowStockWarning.value = true;
      notifStore.addLowStockAlert({
    ...
    toast.success('Transaksi masuk berhasil disimpan.');
    emit('success', {
      productName: product?.name ?? '',
      quantity: qty,
      currentStock: result.current_stock,
    });
    form.value = { product_id: '', quantity: '', transaction_date: today(), supplier_name: '', price_per_unit: '' };
  } catch (err: unknown) {
    ...
    else if (status === 403) formError.value = 'Anda tidak memiliki akses untuk transaksi masuk.';
